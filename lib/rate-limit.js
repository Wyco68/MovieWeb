...
export { getClientIp, isIpBanned, isBlockedUserAgent } from "./edge-guard.js";
...
export const RATE_LIMIT_POLICIES = {
  ...
  TMDB_PAGE: { limit: 40, windowSeconds: 60 },
...
let loggedProductionFallbackWarning = false;
...
function applyFallbackRateLimit(redisKey, policy) {
  ...
  if (isProduction && !loggedProductionFallbackWarning) {
    loggedProductionFallbackWarning = true;
    console.warn(
      JSON.stringify({
        type: "rate_limit_fallback_active",
        message:
          "Upstash Redis unavailable in production — using strict per-instance fallback limits. Configure UPSTASH_REDIS_REST_URL and UPSTASH_REDIS_REST_TOKEN.",
        timestamp: new Date().toISOString(),
      }),
    );
  }

  const fallbackCap = isProduction && !isRedisConfigured()
    ? 5
    : RATE_LIMIT_POLICIES.FALLBACK.limit;
  const limit = Math.min(policy.limit, fallbackCap);
