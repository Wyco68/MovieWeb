...
import { applyRateLimit, RATE_LIMIT_POLICIES } from "./rate-limit";
...
async function enforceSsrRateLimit(endpoint) {
  if (String(endpoint) === "/configuration") return;

  const isSearch = String(endpoint).includes("/search/");
  const policy = isSearch ? RATE_LIMIT_POLICIES.TMDB_SEARCH : RATE_LIMIT_POLICIES.TMDB_API;

  try {
    const headersList = await headers();
    const request = { headers: { get: (name) => headersList.get(name) } };
    ...
    // No request context (e.g. build) — skip rate limiting.
  }
}

export async function tmdbFetch(endpoint, { params = {}, revalidate = 3600, skipRateLimit = false } = {}) {
  if (!skipRateLimit) {
    await enforceSsrRateLimit(endpoint);
  }
