...
export default function MovieDetailView() {
  const [state, setState] = useState({ status: "loading", movie: null, collection: null });
  ...
  useEffect(() => {
    ...
    if (!isValidId(id)) {
      setState({ status: "notfound", movie: null, collection: null });
      return undefined;
    }

    fetchTmdb({ key: "movie_detail", movieId: id })
      .then(async (movie) => {
        if (!movie?.id) throw new Error("not found");
        const collection = movie.belongs_to_collection?.id
          ? await fetchTmdb({ key: "collection", collectionId: movie.belongs_to_collection.id }).catch(() => null)
          : null;
        if (active) setState({ status: "ready", movie, collection });
      })
      .catch(() => {
        if (active) setState({ status: "notfound", movie: null, collection: null });
      });
  ...
  const collectionResult = state.collection;
  ...
  return (
    <div className="pb-16 flex flex-col">
      ...
      <div className="flex flex-col gap-10">
        ...
        <section>
          <Persons imageConfig={imageConfig} credits={movie.credits} />
        </section>
