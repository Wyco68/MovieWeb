...
import { parseRatingNumber, sanitizeSearchPageParams } from "@/lib/search-params";
...
export default async function Search({ searchParams }) {
  ...
  const { q, mediaType, genre, year, language, rating } = sanitizeSearchPageParams(resolvedSearchParams);
  ...
  const filteredResults = filterSearchResults(search?.results ?? [], {
    ...
    genre,
    year,
    minRating: parseRatingNumber(rating),
  });
