...
import { episodeParams } from "@/lib/route-params";
...
async function resolveEpisode(tvId, seasonNumber, episodeNumber) {
  const currentSeasonNumber = Number(seasonNumber);
  const currentEpisodeNumber = Number(episodeNumber);

  const [tv, episode, seasonDetails, episodeVideosResult] = await Promise.all([
    fetchTmdb({ key: "tv_detail", tvId }),
    fetchTmdb({ key: "episode_detail", tvId, season: seasonNumber, episode: episodeNumber }),
    fetchTmdb({ key: "tv_season", tvId, season: seasonNumber }).catch(() => ({ episodes: [] })),
    fetchTmdb({ key: "episode_videos", tvId, season: seasonNumber, episode: episodeNumber }).catch(() => ({ results: [] })),
  ]);

  if (!tv?.id || !episode?.id) return null;
  ...
  if (!previousEpisodeTarget || !nextEpisodeTarget) {
    const [previousSeasonDetails, nextSeasonDetails] = await Promise.all([
      !previousEpisodeTarget && previousSeasonNumber
        ? fetchTmdb({ key: "tv_season", tvId, season: previousSeasonNumber }).catch(() => null)
        : Promise.resolve(null),
      !nextEpisodeTarget && nextSeasonNumber
        ? fetchTmdb({ key: "tv_season", tvId, season: nextSeasonNumber }).catch(() => null)
        : Promise.resolve(null),
    ]);
  ...
  return { tv, episode, episodeVideosResult, previousEpisodeTarget, nextEpisodeTarget };
}

function NotFound({ tvId }) {
  return (
    <>
      <div className="mb-4">
        <BackButton fallbackHref={tvId ? `/tv/${tvId}` : "/tv"} label="Back to Series" alwaysRedirectToFallback />
      </div>
      <h2 className="text-2xl font-semibold">Episode not found</h2>
      <p className="mt-2 muted-label">This episode could not be loaded.</p>
    </>
  );
}

export default function EpisodeDetailView() {
  const [state, setState] = useState({ status: "loading", data: null, params: null });

  useEffect(() => {
    let active = true;
    const { tvId, seasonNumber, episodeNumber } = episodeParams();

    if (
      !isValidId(tvId) ||
      !/^\d{1,3}$/.test(String(seasonNumber)) ||
      !/^\d{1,4}$/.test(String(episodeNumber))
    ) {
      setState({ status: "notfound", data: null, params: { tvId } });
      return undefined;
    }

    resolveEpisode(tvId, seasonNumber, episodeNumber)
      .then((data) => {
        if (!active) return;
        if (!data) {
          setState({ status: "notfound", data: null, params: { tvId } });
        ...
        setState({ status: "ready", data, params: { tvId, seasonNumber, episodeNumber } });
      })
      .catch(() => {
        if (active) setState({ status: "notfound", data: null, params: { tvId } });
      });
  ...
  if (state.status === "notfound") return <NotFound tvId={state.params?.tvId} />;

  const { tvId, seasonNumber, episodeNumber } = state.params;
  const { tv, episode, episodeVideosResult, previousEpisodeTarget, nextEpisodeTarget } = state.data;
